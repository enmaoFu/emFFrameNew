...
public class WaterBottleLoadingRenderer extends LoadingRenderer {
    ...
    private void init(Context context) {
        ...
        mTextSize = DensityUtils.dp2px(context, DEFAULT_TEXT_SIZE);
        ...
        mBottleWidth = DensityUtils.dp2px(context, DEFAULT_BOTTLE_WIDTH);
        mBottleHeight = DensityUtils.dp2px(context, DEFAULT_BOTTLE_HEIGHT);
        mWaterLowestPointToBottleneckDistance = DensityUtils.dp2px(context, WATER_LOWEST_POINT_TO_BOTTLENECK_DISTANCE);
    ...
    protected void draw(Canvas canvas, Rect bounds) {
        ...
        //draw loading text
        mPaint.setColor(DEFAULT_WATER_COLOR);
        canvas.drawText(LOADING_TEXT, mBottleBounds.centerX() - mLoadingBounds.width() / 2.0f,
                mBottleBounds.bottom + mBottleBounds.height() * 0.2f+DensityUtils.dp2px(context, 10), mPaint);
